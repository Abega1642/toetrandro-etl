...
class TestFinalMerge(unittest.TestCase):
    def setUp(self):
        ...
        self.historical_df = pd.DataFrame([
            ["CityA", "2020-01-01", "2020-01-01 06:00", "2020-01-01 18:00", 25, 20, 30, 25, 1010, 60, 3, 180, 5, 20, 0, 0, "Clear", "clear sky", "Nice", "2025-07-04", True, True, True, True, 1.0, True, "January", 2020, "Wednesday"]
        ], columns=self.columns)

        self.new_df = pd.DataFrame([
            ["CityA", "2020-01-02", "2020-01-02 06:00", "2020-01-02 18:00", 26, 21, 31, 26, 1011, 61, 2, 190, 4, 10, 0, 0, "Clouds", "few clouds", "Okay", "2025-07-04", True, True, True, True, 0.9, True, "January", 2020, "Thursday"]
        ], columns=self.columns)
    ...
    def test_merge_adds_new_row(self):
        merger = FinalMerge(str(self.historical_path), str(self.new_data_path), str(self.output_path))
    ...
    def test_backup_created_when_overwriting(self):
        ...
        merger = FinalMerge(str(self.historical_path), str(self.new_data_path), str(self.historical_path))
        merger.apply()
    ...
    def test_merge_sorts_by_city_and_timestamp(self):
        ...
        pd.testing.assert_frame_equal(merged_df.reset_index(drop=True), sorted_df.reset_index(drop=True))
