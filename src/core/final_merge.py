...
class FinalMerge(Process):
    ...
    def __init__(
        ...
        self.historical_path = Path(historical_path) if historical_path else base_dir / "data" / "historical" / "cleaned_historical_data.csv"
        self.new_data_path = Path(new_data_path) if new_data_path else base_dir / "data" / "merged" / "all_weather_data.csv"
        self.output_path = Path(output_path) if output_path else base_dir / "data" / "merged" / "ready_data.csv"

    def apply(self) -> pd.DataFrame:
        logger.info("🔄 FinalMerge: Starting merge process.")
        merged_df = self._merge()
        logger.info(f"✅ FinalMerge: Merge completed. Final dataset contains {len(merged_df)} rows.")
        return merged_df
    ...
    def _merge(self) -> pd.DataFrame:
        ...
        logger.info("🔗 Concatenating datasets and removing duplicates based on ['city', 'timestamp']")
    ...
    def commit(self):
        if self.output_path != self.historical_path:
            ...
            logger.warning("⚠️ Output path is already the historical path. Nothing to commit.")
